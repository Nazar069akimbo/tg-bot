...
async def handle_text(message: types.Message):
    ...
    # Команды обрабатываются отдельными хендлерами (Command("start") и т.д.)
    ...
    # Если пользователь редактирует картинку — запускаем правку
    if state.get("state") == "waiting_edit":
        logger.info(f"📌 [{user_id}] Редактирование картинки")
        from .edit import handle_edit_message
        await handle_edit_message(message)
        return
