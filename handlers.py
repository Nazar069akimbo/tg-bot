...
async def stats_cmd(message: types.Message):
    ...
    text += f"🎯 Режим: {'🧠 Текст' if user_modes.get(user_id, 'text') == 'text' else '🖼️ Картинка'}"
...
async def handle_message(message: types.Message):
    ...
    # Проверяем состояние контакта
    contact_state = user_pages.get(user_id, {})
...
async def generate_text(message: types.Message):
    user_id = message.from_user.id
    ok, remaining = can_request(user_id)
    if not ok:
...
async def handle_admin_input(message: types.Message):
    ...
    if state.get("state") == "waiting_premium_user":
        try:
            ...
            await message.answer(f"❌ Ошибка: {e}\nФормат: ID дни", reply_markup=admin_kb())
